...
export default function GroupsPage() {
  ...
  const fetchGroups = async () => {
    setLoading(true);
    try {
      // 🔥 FIX: Order by createdAt (which ALL groups have) instead of lastActivity
      const q = query(
      ...
      // 🔥 SORT CLIENT-SIDE: Groups with recent activity float to the top!
      groupsData.sort((a, b) => {
      ...
      // 🔥 AUTO-BACKFILL: Add lastActivity to old groups that don't have it
      groupsData.forEach(async (group) => {
        if (!group.lastActivity && group.createdAt) {
          try {
  ...
  return (
    // 🔥 WRAPPED IN PROTECTED ROUTE SO THE BOTTOM BAR STAYS!
    <ProtectedRoute>
